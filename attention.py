...
class Attention(Wrapper):
    ...
    def build(self, input_shape):
        ...
        layer_output_dim = self.layer.compute_output_shape(input_shape)
        self.W1 = self.layer.__init__((input_dim, input_dim, 1, 1), name='{}_W1'.format(self.name))
        self.W2 = self.layer.__init__((layer_output_dim, input_dim), name='{}_W2'.format(self.name))
        self.b2 = K.zeros((input_dim,), name='{}_b2'.format(self.name))
        self.W3 = self.layer.__init__((input_dim*2, input_dim), name='{}_W3'.format(self.name))
        self.b3 = K.zeros((input_dim,), name='{}_b3'.format(self.name))
        self.V = self.layer.__init__((input_dim,), name='{}_V'.format(self.name))

        print(self.W1, self.W2, self.W3, self.V, self.b2, self.b3)
...
if __name__ == "__main__":
    ...
    print("base line test with LSTM")
    model = Sequential()
    model.add(InputLayer(batch_input_shape=(nb_samples, timesteps, embedding_dim)))
    model.add(Attention(recurrent.LSTM(output_dim, implementation=1, input_shape=(None, embedding_dim), return_sequences=True)))
    model.add(core.Activation('relu'))
    model.compile(optimizer='rmsprop', loss='mse')
    model.fit(x,y, nb_epoch=1, batch_size=nb_samples)


    print("test stacked with all RNN layers and consume_less options")
    model = Sequential()
    model.add(InputLayer(batch_input_shape=(nb_samples, timesteps, embedding_dim)))

    print("test supported consume_less options")
    ...
    print("test each other RNN type")
    ...
    print("test with return_sequence = False")
    model = Sequential()
    model.add(InputLayer(batch_input_shape=(nb_samples, timesteps, embedding_dim)))
    ...
    print("with bidirectional encoder")
    model = Sequential()
    model.add(InputLayer(batch_input_shape=(nb_samples, timesteps, embedding_dim)))
